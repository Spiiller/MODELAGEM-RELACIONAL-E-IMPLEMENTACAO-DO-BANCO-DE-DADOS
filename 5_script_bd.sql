...
DROP SEQUENCE sequence_cd_produto;

DROP SEQUENCE sequence_ctg_produto;

DROP SEQUENCE sequence_cd_chamado;
...
CREATE TABLE mc_cliente (
    ...
    tipo_cliente2      VARCHAR2(2) NOT NULL
);

ALTER TABLE mc_cliente
    ADD CONSTRAINT arc_1_lov CHECK ( tipo_cliente IN ( 'cliente_pj', 'cliente_pf' ) );

COMMENT ON COLUMN mc_cliente.cd_cliente IS
    'Código do cliente';

COMMENT ON COLUMN mc_cliente.tipo_cliente IS
...
COMMENT ON COLUMN mc_cliente.tipo_cliente2 IS
    'Define se é pessoa física (PF) ou jurídica (PJ)';
...
CREATE SEQUENCE sequence_ctg_produto
START WITH 1
INCREMENT BY 1
NOCACHE
ORDER;
...
ALTER TABLE mc_funcionario ADD CONSTRAINT pk_funcionario PRIMARY KEY ( cd_func );
...
CREATE SEQUENCE sequence_cd_produto
START WITH 1
INCREMENT BY 1
NOCACHE
ORDER;
...
ALTER TABLE mc_cliente_pj
    ADD CONSTRAINT pode_serv2 FOREIGN KEY ( cd_cliente )
        REFERENCES mc_cliente ( cd_cliente );
...
BEGIN
    ...
    IF ( d IS NULL OR d <> 'cliente_pj' ) THEN
        raise_application_error(-20223, 'FK pode_serv2 in Table MC_CLIENTE_PJ violates Arc constraint on Table MC_CLIENTE - discriminator column tipo_cliente doesn''t have value ''cliente_pj'''
        );
    END IF;
...
BEGIN
    ...
    IF ( d IS NULL OR d <> 'cliente_pf' ) THEN
        raise_application_error(-20223, 'FK pode_ser in Table MC_CLIENTE_PF violates Arc constraint on Table MC_CLIENTE - discriminator column tipo_cliente doesn''t have value ''cliente_pf'''
        );
    END IF;
...
BEGIN
    ...
    IF ( d IS NULL OR d <> 'tipo_reclamacao' ) THEN
        raise_application_error(-20223, 'FK classificarv4 in Table MC_SAC_CHAMADO_RECLAMACAO violates Arc constraint on Table MC_SAC_CHAMADO - discriminator column cd_tipo_chamado doesn''t have value ''tipo_reclamacao'''
        );
    END IF;
...
BEGIN
    ...
    IF ( d IS NULL OR d <> 'tipo_sugestao' ) THEN
        raise_application_error(-20223, 'FK classificarv3 in Table MC_SAC_CHAMADO_SUGESTAO violates Arc constraint on Table MC_SAC_CHAMADO - discriminator column cd_tipo_chamado doesn''t have value ''tipo_sugestao'''
        );
    END IF;
